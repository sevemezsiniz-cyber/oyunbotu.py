...
def is_incoming_blocked(text: str) -> bool:
    """Hedef bot'tan gelen mesajda kullanıcı adı veya link var mı?"""
    if not text:
        return False
    
    # Tüm Telegram kullanıcı adlarını yakala (@ ile başlayan)
    if re.search(r'@[\w_]+', text):
        return True
    
    # Tüm linkleri yakala (http, https, t.me, telegram.me vb.)
...
async def target_bot_handler(event):
    ...
    try:
        ...
        # === DOSYA GELDİYSE (ÖNCE KONTROL ET) ===
        if event.message.file:
            filename = event.message.file.name
            
            # Sadece izin verilen TXT dosyalarını ilet
            if filename and filename in ALLOWED_TXT_FILES:
                print(f"📁 Dosya alındı ve iletiliyor: {filename}")
                
                buffer = io.BytesIO()
                await event.download_media(buffer)
                buffer.seek(0)

                # Dosyayı gönder (içinde link veya kullanıcı adı olsa bile)
                await bot_app.bot.send_document(
                    chat_id=current_user,
                    document=InputFile(buffer, filename=filename),
                    caption="📄 Sorgu sonucu"
                )
                print(f"✅ Dosya iletildi: {filename}")
                return  # İşlem tamam, çık
            
            else:
                print(f"⚠️ İzin verilmeyen dosya: {filename}")
                return
        
        # === METİN GELDİYSE (DOSYA DEĞİLSE) ===
        if raw_text:
            # Kullanıcı adı veya link varsa İLETME
            ...
            print(f"✅ Mesaj iletildi: {raw_text[:100]}")
